...
/* Egyptian local mobile (01XXXXXXXXX) -> international digits for a
   wa.me link (20 + number without the leading 0). Falls back to the raw
   phone string if it doesn't look like the expected local format. */
...
function renderTable(rows){
  ...
  if (!rows.length) {
    tbody.innerHTML = '<tr><td colspan="10" class="empty-state">' + veloraT("admin.empty") + '</td></tr>';
    return;
  }
  tbody.innerHTML = rows.map(r => {
    ...
    const phoneCell = r.phone
      ? '<td><a class="contact-link" href="tel:' + escapeHtml(r.phone) + '">' + escapeHtml(r.phone) + '</a> ' +
        '<a class="contact-link wa-link" href="https://wa.me/' + escapeHtml(waNumber(r.phone)) + '" target="_blank" rel="noopener" title="WhatsApp">💬</a></td>'
      : "<td>-</td>";
    const emailCell = r.email
      ? '<td><a class="contact-link" href="mailto:' + escapeHtml(r.email) + '">' + escapeHtml(r.email) + "</a></td>"
      : "<td>-</td>";
    return "<tr>" +
      ...
      '<td><button type="button" class="btn-icon del-row-btn" data-id="' + escapeHtml(r.id) + '" title="' + veloraT("admin.deleteBtn") + '">🗑</button></td>' +
      "</tr>";
  }).join("");

  tbody.querySelectorAll(".del-row-btn").forEach(btn => {
    btn.addEventListener("click", () => deleteBookingRow(btn.getAttribute("data-id")));
  });
}

async function deleteBookingRow(id){
  if (!window.confirm(veloraT("admin.deleteConfirm"))) return;
  try {
    const { doc, deleteDoc } = window.veloraFirestoreMod;
    await deleteDoc(doc(window.veloraDb, "bookings", id));
    veloraAllRows = veloraAllRows.filter(r => r.id !== id);
    const upcoming = renderStats(veloraAllRows);
    renderUpcoming(upcoming);
    applyFilterSortAndRender();
  } catch (err) {
    console.error("Velora booking delete error:", err);
    alert(veloraT("admin.deleteError"));
  }
}
